...
def get_job_definition(blockdevice):
    ...
    for dev in blockdevice:
        volumes.append(
            {"name": f"pvc-{dev}", "persistentVolumeClaim": {"claimName": f"pvc-{dev}"}}
        )

    job = {
        "apiVersion": "batch/v1",
        "kind": "Job",
        "metadata": {"name": "fio-benchmarks"},
        "spec": {
            "template": {
                "metadata": {"labels": {"app": "fio-benchmarks"}},
                "spec": {
                    "containers": [
                        {
                            "name": "fio-container",
...
def write_definitions(size, blockdevice, hostname):
    ...
    for dev in blockdevice:
        pv_definition.append(
            yaml.dump(get_pv_definition(f"pv-{dev}", size, dev, hostname))
        )
        pvc_definition.append(yaml.dump(get_pvc_definition(f"pvc-{dev}", size, dev)))
