...
# usage: bash ramdisk_ceph.sh 4 myclusterkey.id_rsa  ubuntu;
# by default each osd 100G of ramdisk
...
for ((i = 0 ; i < ${nosds} ; i++)); do
    ...
    ssh osd${i} "sudo mkdir /mnt/mnttmp; sudo mount -t tmpfs -o size=100G tmpfs /mnt/mnttmp; sudo truncate -s 100G /mnt/mnttmp/loopfile; sudo losetup -f /mnt/mnttmp/loopfile; sudo losetup -j /mnt/mnttmp/loopfile;"
done;
echo "";
sleep 2s;
...
for ((i = 0 ; i < ${nosds} ; i++)); do
    ...
    sudo env/bin/ceph-deploy --overwrite-conf osd create osd${i}:/dev/loop0
